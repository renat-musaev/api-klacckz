...
class BookController extends Controller
{
    ...
    public function gdz(Request $request)
    {
        ...
        $with['subject'] = function($query) {
            ...
            ])->select(['id', 'classroom_id', 'name', 'show_pages', 'show_video', 'show_lessons']);
        };
        ...
        $book = Book::with($with)->where([
            ...
            'id', 'subject_id', 'name', 'cover', 'show_pages', 'show_video', 'show_lessons',
    ...
    // Получить книгу по ID
    public function videoSolutions(Request $request)
    {
        if (auth()->guard('api')->user()) {
            VideoPayment::where([
                'user_id' => auth()->guard('api')->user()->id,
    ...
    // Получить книгу по ID
    public function videoLessons(Request $request)
    {
        if (auth()->guard('api')->user()) {
            LessonPayment::where([
                'user_id' => auth()->guard('api')->user()->id,
        ...
        $with['lessons'] = function($query) {
            $query->with('tests')->select(['id', 'book_id', 'name', 'link', 'free', 'key', 'task']);
        };
